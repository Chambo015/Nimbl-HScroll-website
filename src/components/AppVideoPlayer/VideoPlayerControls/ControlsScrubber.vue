...
<template>
  <div ref="scrubber" class="relative h-[6px] duration-200 cursor-pointer select-none bg-black dark:bg-white dark:bg-opacity-10 bg-opacity-20" @mousedown="scrubbing = true">
    <div class="relative overflow-hidden h-full w-full ">
      <div class="h-full absolute opacity-30 left-0 top-0 bg-blue-600 w-full " :style="{ transform: `translateX(${secondary / max * 100 - 100}%)` }" />
      <div class="relative h-full w-full bg-blue-200 " :style="{ transform: `translateX(${value / max * 100 - 100}%)` }" />
    </div>
